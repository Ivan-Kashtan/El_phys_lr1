...
f = [KeepTogether([mn, t_u, un, s_80, kaf, s_80, tp, title, var, s_130, fac, gr, st, tchr, mk, s_130, sity]),
     ...
     fml('$e(t) = sin \\left(\\omega t \\right)$'),
     sp_20,
     fml(f'$U_{{пер}} = \\sqrt {{({u_c_f[0]} - {u0})^2 + {z_c:.0f}^2 \\cdot ({i_l_f[1]:.4f} - {i0})^2}} = {abs(u_p[1]):.2f}$ В'),
     sp_25,
     fml(f'$I_{{пер}} = \\sqrt {{({u_c_f[0]:.2f} - {u0})^2 / {z_c:.0f}^2 + ({i_l_f[1]:.4f} - {i0})^2}}$ = {abs(i_f[1]):.4f} А'),
     sp_25,
     ...
     fml(f'$i_{{вын}} = \\dfrac {{{e_a[0]}}} {{{z_f:.1f}}} '
         f'= {polar(i_f[1])[0] * 10 ** 3:.1f} \u2220 {polar(i_f[1])[1]:.1f}$ мА'),
     sp_30,
